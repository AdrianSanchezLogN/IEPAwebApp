      ...
      if (! isNaN(id)) {
        ...
        db.collection('category', function(err, collection_category) {
          if(skip<0){
            ...
            collection_category.find().count(function(err, value){
              collection_category.find().skip(parseInt((skip-1)*5)).limit(5).toArray(function(err, items) {
          ...
          collection_user.update({recentCategories: parseInt(id)}, {$pull: {recentCategories: parseInt(id)}}, {safe: true}, function(err, result) {
            if (err) {
              ...
              db.collection('product', function(err, collection_product){
                collection_product.update({category: parseInt(id), 'category.1': {$exists: 1}}, {$pull: {category: parseInt(id)}}, {safe: true}, function(err, result) {
                  if (err) {
                    ...
                    collection_product.findOne({category: parseInt(id)}, function(err, items_left) {
                      if(items_left != null){
