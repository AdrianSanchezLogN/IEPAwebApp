                                ...
                                db.collection('user', function(err, collection_user) {
                                    ...
                                    collection_user.findOne({email: item._id, password: password}, function(err, items){
                                        if(items != null){
                                            ...
                                            var session_token = '#' + items._id + '-' + (new Date().getTime());
                                            collection_user.update({_id: items._id}, {$set: {sessionValue: session_token, sessionDate: (new Date()), sessionLogIn: (new Date())}}, {safe: true}, function(err, result) {
                                                if (err) {
                                            ...
                                            res.send(200, {'User' : items.name, 'id' : items._id, 'session_token' : session_token}); 
                                        }
                                        else {
...
/**
* @description Login an admin in the database (setting the sessionValue)
* @author Adrián Sánchez <adriansanchez.logn@gmail.com>
* @param {req, res} request (with the email and password) send to the server and the response return by the server
* @returns {Int} session_token
*/ 
                    ...
                    try {
                        collection_email.findOne({email: email}, function(err, item) {
                            if(item != null){
                                db.collection('user', function(err, collection_user) {
                                    password = new Buffer(password, 'utf8');
                                    collection_user.findOne({email: item._id, password: password, admin: 1}, function(err, items){
                                        if(items != null){                                            
                                            var session_token = '#' + items._id + '-' + (new Date().getTime());
                                            console.log('User accepted');
                                            collection_user.update({_id: items._id}, {$set: {sessionValue: session_token, sessionDate: (new Date()), sessionLogIn: (new Date())}}, {safe: true}, function(err, result) {
